...
export async function verificationView(s,now=Date.now()){const v=state(s),evaluations=[];for(const c of current(v.contracts))evaluations.push(await evaluateContract(s,c.key,now));const reports=v.reports.map(r=>({...r,needsRefresh:r.body.evaluations.some(old=>{const cur=evaluations.find(e=>e.contractId===old.contractId);return !cur||cur.contractHash!==old.contractHash||canonical(cur.checks)!==canonical(old.checks);})}));return {contracts:current(v.contracts),evidence:current(v.evidence),evaluations,reports,templates:reportTemplates(s),historyCount:v.evaluations.length,alerts:current(v.alerts||[]),lastReconciled:v.lastReconciled||null,mode:'UNVERIFIED_DIAGNOSTICS'};}
