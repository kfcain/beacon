...
Prefer STS assume-role for `BeaconWriter` (Put/Get/List, no DeleteObject) and `BeaconAuditor` (read-only). See [docs/STORAGE.md](docs/STORAGE.md), [deploy/aws/README.md](deploy/aws/README.md), and the architecture notes in [docs/architecture/beacon-evidence-lake.md](docs/architecture/beacon-evidence-lake.md).
