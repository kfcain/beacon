...
- `beacon push` writes a local sealed pack (records, checkpoints, public keys). When `BEACON_S3_BUCKET` is set, it also writes the pack to the AWS evidence lake. It does not upload to Paramify or any other GRC SaaS.
...
- Remote S3/DynamoDB storage does not prove that evidence content is true. It stores sealed bytes, hashes, and an index. It is not an audit opinion.
