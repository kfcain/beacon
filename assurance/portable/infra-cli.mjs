...
try{
 ...
 if(command==='demo'){const result=await repo.mutate(owner,'infra-demo',{scenario:value||'drift'});console.log(JSON.stringify(result.output,null,2));}
 ...
 else if(command==='gate'||command==='evaluate'){
  ...
  if(command==='gate')throw new Error('Infrastructure gate blocked: trusted admission is not implemented. Use evaluate for configuration diagnostics; imported, simulated or self-labeled observations cannot authorize deployment.');
  console.log(JSON.stringify({schema:'beacon.inventory.gate.v1',expected:boundary.manifest.resources,resources:run.observation.resources,ageSeconds:(Date.now()-Date.parse(run.observation.collectedAt))/1000,maxAgeSeconds:boundary.manifest.maxAgeHours*3600,context:{purpose:'configuration-evaluation',manifestHash:boundary.hash,observationHash:run.hash,stage,provenance:run.provenance}},null,2));
 }else throw new Error('Usage: infra-cli.mjs demo SCENARIO | register MANIFEST.json | import OBSERVATION.json | status | evaluate BOUNDARY_ID planned|observed | gate BOUNDARY_ID planned|observed');
