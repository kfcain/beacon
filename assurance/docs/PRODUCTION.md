...
| Policy/procedure mapping | GRC PDF Mapper report import; private upload adapter; statement/candidate inspection; version comparison; provider mapping reviews and claim links; read-only MCP | Private HTTPS mapper deployment with gateway authentication, PDF/DOCX/OCR exercise, quarantined original-file storage and formal approvals |
...
Dependency review on 2026-09-07: npm reported 23 advisories (17 high, 5 moderate, 1 low) before remediation and zero after a clean lockfile installation. Updated pinned Next/React RSC, Vite/Cloudflare tooling, and Vinext; Vinext `1.0.0-beta.9` removes the vulnerable image-size dependency, while a scoped esbuild override fixes the older Drizzle loader dependency. This is a framework prerelease upgrade and needs deployment/browser validation before release. TypeScript, both application builds, 36 Node tests and Drizzle configuration/schema generation passed; no schema changes were generated. CI now fails on high/critical npm advisories. Zero reported npm advisories is not a full security assessment or a container/OS scan.

Relevant maintainer advisory: [Next.js Server Actions denial of service](https://github.com/vercel/next.js/security/advisories/GHSA-m99w-x7hq-7vfj). The lockfile is the exact dependency record; the gate uses the registry's current advisory dataset.
