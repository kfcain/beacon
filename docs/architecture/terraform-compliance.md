...
| Writer reads lake logs | When the logging bucket exists, writer SIDs `S3ListLakeLogs` and `S3GetLakeLogs` allow List/Get only on `s3-access-logs/` and `cloudtrail/`. No Put or Delete on that bucket. Auditor has no access. | `aws.lake.logs` can read raw logs, then seal them into the evidence bucket. | Deny a logging-bucket Allow that is not that read. Deny when the logging bucket exists and those SIDs are missing. |
...
| Prefix and partition bind | Writer and auditor S3 object ARNs use `${local.workspace_prefix}/*`. DynamoDB uses `dynamodb:LeadingKeys` = `${local.ddb_pk}` (`{tenant}#{workspace}`). | IAM matches `BEACON_TENANT_ID` / `BEACON_WORKSPACE_ID`. | Deny if any S3 object Allow resource is not prefix-scoped, or if any DynamoDB item Allow statement omits LeadingKeys. |
| IAM Allow wildcards | Writer and auditor allow lists use precise actions. Service-wide `s3:*` / `dynamodb:*` / `kms:*` and `*` are absent. | Least privilege. Partial wildcards such as `s3:Delete*` must not grant delete. | Deny if an Allow pattern matches a forbidden action (IAM glob) or is a service-wide wildcard. |
| Evidence bucket policy attachment | `aws_s3_bucket_policy.evidence` sets `bucket = aws_s3_bucket.evidence.id` and `policy = data.aws_iam_policy_document.bucket.json`. | Deny statements must apply to the evidence bucket. | Deny unless that attachment binds the named bucket document to `aws_s3_bucket.evidence`. |
| TLS in transit | SID `DenyInsecureTransport` denies `s3:*` when `aws:SecureTransport` is false. | HTTPS only. | Deny if the SID is missing or the statement does not deny `s3:*` for principal `*` on the evidence bucket when `aws:SecureTransport` is false. |
| S3 access logging | `aws_s3_bucket_logging.evidence` writes to `aws_s3_bucket.logs` prefix `s3-access-logs/`. Default on (`enable_s3_access_logging`). | Raw access observations for later seal. | Deny if logging is missing or targets the evidence bucket. |
| CloudTrail data events | `aws_cloudtrail.evidence` logs `AWS::S3::Object` read and write on the evidence bucket. `include_management_events = false`. Default on (`enable_cloudtrail_data_events`). | Object-level lake audit. Not an account management trail. | Deny if the trail is missing or enables management events. |
...
| Seal access logs / CloudTrail as Beacon objects | `aws.lake.logs` reads the logging bucket and seals an observation plus a finding. Raw AWS objects stay in the logging bucket. | Keep class separation. Do not copy raw log objects into `observations/` or `evidence/`. |
