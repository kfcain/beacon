...
| Signing and proof | Separate KMS signing utility; offline verification against a supplied trusted-key registry; job/image/scope/evidence binding; substitution and replay tests | KMS key, key policy, authorized-job registry, source authentication in signer, external monotonic checkpoint and key rotation/revocation operations |
...
| CI/CD | Executable fail-closed CLI; sample production gate; code-validation workflow | Trusted runner/artifact path, protected policy inputs, OIDC roles, approved action/image digests, registry admission and deployment verification |
