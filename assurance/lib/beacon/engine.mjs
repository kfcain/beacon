...
export async function snapshot(state,now=Date.now()){
 const s=structuredClone(state);for(const c of s.claims){const r=latest(s,c.id);if(r)r.evaluation=await evaluate(c,r.raw,c.scope,now);c.current=r?{...r.evaluation,runId:r.id,at:earliestObservation(r),provenance:r.provenance,review:r.review}:{status:'UNKNOWN',coverage:0,expected:c.scope.length,reasons:['Evidence and review required']};}return s;
}
const text=(v,max=12000)=>{assert(typeof v==='string'&&v.trim().length>0&&v.length<=max,'Invalid text');return v.trim();};
export const ACTIONS=['run','import','review','document','publish','reconcile','configure','relay','requirement'];
