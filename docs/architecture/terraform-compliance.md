...
| Seal access logs / CloudTrail as Beacon objects | Logging bucket holds raw AWS files. No collector copies them into `observations/` then `evidence/`. | Add a collector that ingests log objects as observations and seals findings. Keep class separation. |
